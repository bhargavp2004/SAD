...
def create_model():
    ...
    model.add(TimeDistributed(Flatten()))  # Ensure output is a sequence
    model.add(Bidirectional(LSTM(32)))  # BiLSTM layer without return_sequences=True
...
plt.title('Training and Validation Accuracy for CONV2D+CONVLSTM2D+BILSTM ')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
plt.show()
...
plt.title('Training and Validation Loss for CONV2D+CONVLSTM2D+BILSTM ')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.show()
...
print(f'Test Accuracy (CONV2D+CONVLSTM2D+BILSTM): {test_accuracy:.4f}')
...
f1 = f1_score(labels_test_classes, predictions_classes, average='weighted')
...
model.save("Suspicious_Human_Activity_Detection_CONV2D+CONVLSTM2D+BILSTM.keras")

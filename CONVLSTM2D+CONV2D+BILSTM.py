...
def create_model():
    ...
    model.add(ConvLSTM2D(32, kernel_size=(3, 3), activation='relu', input_shape=(SEQUENCE_LENGTH, IMAGE_HEIGHT, IMAGE_WIDTH, 3), padding='same', return_sequences=True))
...
plt.title('Training and Validation Accuracy for CONVLSTM2D+CONV2D+BILSTM')
plt.xlabel('Epoch')
plt.ylabel('Accuracy')
plt.legend()
plt.show()
...
plt.title('Training and Validation Loss for CONVLSTM2D+CONV2D+BILSTM ')
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.legend()
plt.show()
...
print(f'Test Accuracy (CONVLSTM2D+CONV2D+BILSTM): {test_accuracy:.4f}')
